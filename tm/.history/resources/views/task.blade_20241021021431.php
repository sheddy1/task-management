...
<body>
    ...
    <div class="project_header">
        ...
        <form action="{{ route('main') }}">
            <select name="" id="" class="project_header_select" cid="project_header_select">
                <option value="">Select Project</option>
                @foreach($project as $key => $data)
                    <option value="{{ $data->unique_id }}">{{ $data->name }}</button></option>
                @endforeach
            </select>
        </form>
...
<script>
    ...
    $('#project_header_select').on('change', function() {
        alert("awesome");
    })
